...
[Felix Wechsler[^1]](https://felixwechsler.science/), Carlo Gigli, Jorge Madrid-Wolff, Christophe Moser

[^1] PhD student at EPFL in Switzerland

!!! error "Abstract"
	We introduce an optimization framework for ray and wave optical tomographic volumetric additive manufacturing (TVAM).

	In TVAM, tomographic patterns are projected with a light modulator onto a photocurable resin from different angular directions. Once an energy dose threshold is crossed, the resin starts polymerizing.

	Current approaches assume a ray optical model for light propagation, using the Radon transform as backbone, which breaks down for small features in the region of $20\mathrm{\textmu m}$. 
...
sinogram_j = radon(julia_logo, angles);
...
git-tree-sha1 = "3c75e24af3bf353c3222e33465f2fc79daabcd2d"
